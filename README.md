# Programacion_Estructurada_IE_2026-2
Repo de la clase de programacion estructurada de ingenieria en electronica del TSJ Zapopan
# TSJ UNIDAD ACADÉMICA ZAPOPAN
![imagen](/images/logo.jpg)


---

### **PORTADA DE TRABAJO Y PORTAFOLIO DE EVIDENCIAS**

---

| **Datos del Curso / Alumno** | **Detalle** |
| :--- | :--- |
| **Institución:** | TSJ Unidad Académica Zapopan |
| **Materia:** | Programación Estructurada |
| **Semestre:** | 3.er Semestre |
| **Periodo Escolar:** | Agosto - Diciembre 2026 |
| **Alumno:** | Alan Laris Diaz |

---

## 📚 Unidades del Curso (Portafolio de Documentación)

A continuación se presentan las unidades temáticas correspondientes al programa de **Programación Estructurada**. Haz clic en cada enlace para acceder a la documentación detallada en su respectivo archivo `.md`:

- [x] 📘 **[Unidad 1: Introducción a la Programación Estructurada](/unidad1/Actividad.md)
  - *Conceptos básicos, algoritmos, diagramas de flujo y pseudocódigo.*

- [ ] 📗 **[Unidad 2: Elementos Básicos del Lenguaje y Tipos de Datos](./Unidad2_ElementosBasicos.md)**
  - *Variables, constantes, operadores aritméticos, lógicos y relacionales.*

- [ ] 📙 **[Unidad 3: Estructuras de Control Condicionales](./Unidad3_EstructurasCondicionales.md)**
  - *Toma de decisiones: `if`, `if-else`, `switch` / `según`.*

- [ ] 📕 **[Unidad 4: Estructuras de Control Repetitivas](./Unidad4_EstructurasRepetitivas.md)**
  - *Bucles e iteraciones: `while`, `do-while`, `for` / `para`.*

- [ ] 📓 **[Unidad 5: Funciones y Modularidad](./Unidad5_FuncionesYModularidad.md)**
  - *Diseño descendente, paso de parámetros por valor y referencia, alcance de variables.*

- [ ] 📔 **[Unidad 6: Arreglos y Estructuras de Datos Básicas](./Unidad6_ArreglosYEstructuras.md)**
  - *Vectores unidimensionales, matrices bidimensionales y registros.*

 **Agosto - Diciembre 2026**.